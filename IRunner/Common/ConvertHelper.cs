...
namespace IRunner
{
    public class ConvertHelper
    {
        ...
        /// SQL注入过滤
        /// </summary>
        /// <param name="strSQL">待检测SQL语句</param>
        /// <returns>安全SQL</returns>
        //tips：若不使用存储过程参数传递，则用此方法过滤非法参数(要使用sp_executesql不要使用：exec)
        public static string PickSQL(string sSqlParameter)
        {
            sSqlParameter = (sSqlParameter == null || sSqlParameter.Length == 0) ? "" : sSqlParameter.Replace("'", "''");//单引号替换成两个单引号
